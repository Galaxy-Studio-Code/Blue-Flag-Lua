...
Recovery_Function:add_action("生成 Ped 掉落 ($2000)", function()
	...
	globals.set_float(2783345 + 5, position.z + 10)
	globals.set_uint(4528329 + 1 + (globals.get_int(2783345) * 85) + 66 + 2, 2)
	globals.set_uint(2783351, 1)
	sleep(1)
end)
Recovery_Function:add_action("生成钱袋 ($10000)", function()
	if not localplayer then
		return nil
	end
	local position = localplayer:get_position()
	globals.set_int(2783345 + 1, 10000)
	globals.set_float(2783345 + 3, position.x)
	globals.set_float(2783345 + 4, position.y)
	globals.set_float(2783345 + 5, position.z + 10)
	globals.set_uint(4528329 + 1 + (globals.get_int(2783345) * 85) + 66 + 2, 2)
	...
	for p in replayinterface.get_pickups() do
		...
		if p:get_pickup_hash() ~= joaat("PICKUP_MONEY_VARIABLE") and p:get_model_hash() == joaat("p_poly_bag_01_s") then
			p:set_pickup_hash("PICKUP_MONEY_VARIABLE")
			break
		end
		::continue::
	end
...
Recovery_Function:add_float_range("最小任务金额", 10000.0, 0.0, 1000000.0, function()
	globals.get_float(262145 + 2424)
end, function(value)
	globals.set_float(262145 + 2424, value)
end)
...
FAST_PERICO_QUICK_FINISH:add_action("立即结算佩里科岛抢劫", function()
	script("fm_mission_controller_2020"):set_int(38257, 102)
...
local FAST_CAH_QUICK_FINISH = FAST_CAH:add_submenu("10秒完成名钻赌场抢劫")
do
	local FAST_CAH_QUICK_FINISH_RunOnce = { { "H3_COMPLETEDPOSIX", -1 }, { "H3OPT_MASKS", 9 }, { "H3OPT_WEAPS", 1 }, { "H3OPT_VEHS", 3 } }
	local FAST_CAH_QUICK_FINISH_PRESET = { { "CAS_HEIST_FLOW", -1 }, { "H3OPT_APPROACH", 3 }, { "H3OPT_TARGET", 1 }, { "H3OPT_POI", 1023 }, { "H3OPT_ACCESSPOINTS", 2047 }, { "H3OPT_DISRUPTSHIP", 3 }, { "H3OPT_BODYARMORLVL", -1 }, { "H3OPT_KEYLEVELS", 2 }, { "H3OPT_CREWWEAP", 4 }, { "H3OPT_CREWDRIVER", 3 }, { "H3OPT_CREWHACKER", 4 }, { "H3OPT_BITSET1", 799 }, { "H3OPT_BITSET0", 3670102 }, { "H3_LAST_APPROACH", 0 }, { "H3_HARD_APPROACH", 0 } }
	FAST_CAH_QUICK_FINISH:add_action("气势汹汹 (黄金,请选择低级买家)", function()
		for i = 1, #FAST_CAH_QUICK_FINISH_RunOnce do
			stat_set_int(FAST_CAH_QUICK_FINISH_RunOnce[i][1], true, FAST_CAH_QUICK_FINISH_RunOnce[i][2])
		end
		for i = 1, #FAST_CAH_QUICK_FINISH_PRESET do
			stat_set_int(FAST_CAH_QUICK_FINISH_PRESET[i][1], true, FAST_CAH_QUICK_FINISH_PRESET[i][2])
		end
...
FAST_CAH_QUICK_FINISH:add_action("立即结算名钻赌场抢劫", function()
	script("fm_mission_controller"):set_int(28271, 99999)
	script("fm_mission_controller"):set_int(28601, 99999)
	script("fm_mission_controller"):set_int(31595, 99999)
end)
do
...
DOOMS_HEIST:add_action("立即结算末日抢劫", function()
	script("fm_mission_controller"):set_int(28271, 99999)
	script("fm_mission_controller"):set_int(28601, 99999)
	script("fm_mission_controller"):set_int(31595, 99999)
end)
...
CLASSIC_HEISTS:add_action("立即结算公寓抢劫", function()
	script("fm_mission_controller"):set_int(28271, 99999)
	script("fm_mission_controller"):set_int(28601, 99999)
	script("fm_mission_controller"):set_int(31595, 99999)
end)
...
LS_ROBBERY:add_action("立即结算改装铺合约", function()
	script("fm_mission_controller_2020"):set_int(38257, 102)
	script("fm_mission_controller_2020"):set_int(36882, 9)
end)
do
